...
def fix_identifier(ustring,idtype=None,online_method=None,regexp=None):
    idtype = str(idtype)
    if regexp:
    ...
    if match:
        ...
        isFound = True
        if online_method:
            online_res = online_method(the_id)
            if not online_res:
                isFound = False
...
def identifier_has_location(idstring,idtype=None,urlbase=None,timeout=10,headers={'User-Agent': 'Mozilla/5.0 (Windows NT 6.0; WOW64; rv:24.0) Gecko/20100101 Firefox/24.0'},proxies={}):
    ...
    try:
        ...
        print('WARNING: "requests.get()" raised an exception for ' + idstring + ', treated as not found')
        isFound = False
    return isFound

def doi_has_content(ustring):
    return identifier_has_location(ustring,idtype='doi')

def pmid_has_content(ustring):
    return identifier_has_location(ustring,idtype='pmid')

def issn_has_content(ustring):
...
def isbn_has_content(ustring,verbose=False):
    #Special case of an empty string (some services can return a false positive)
    if not(ustring):
        return ""
    isbncand = ustring.replace('-','')
    LocList = ["Google Books API", "Open Library Book API", "ISBN search", "Books by ISBN"]
    acc = 0
    try:
        req = requests.get("https://www.googleapis.com/books/v1/volumes?q=isbn:" + isbncand, timeout=10, headers={'User-Agent': 'Mozilla/5.0 (Windows NT 6.0; WOW64; rv:24.0) Gecko/20100101 Firefox/24.0'}, proxies={})
        isFound = req.json()["totalItems"] > 0
    except:
        print('WARNING: "requests.get()" raised an exception for ' + ustring + ', treated as not found')
        isFound = False
    if not isFound:
        acc += 1
        try:
            req = requests.get("https://openlibrary.org/api/books?bibkeys=ISBN:" + isbncand + "&format=json", timeout=10, headers={'User-Agent': 'Mozilla/5.0 (Windows NT 6.0; WOW64; rv:24.0) Gecko/20100101 Firefox/24.0'}, proxies={})
    ...
    if not isFound:
        acc += 1
        try:
            req = requests.get("https://isbnsearch.org/isbn/" + isbncand, timeout=10, headers={'User-Agent': 'Mozilla/5.0 (Windows NT 6.0; WOW64; rv:24.0) Gecko/20100101 Firefox/24.0'}, proxies={})
            isFound = req.status_code == 200
        except:
            print('WARNING: "requests.get()" raised an exception for ' + ustring + ', treated as not found')
    if not isFound:
        acc += 1
        try:
            req = requests.get("https://www.books-by-isbn.com/" + isbncand, timeout=10, headers={'User-Agent': 'Mozilla/5.0 (Windows NT 6.0; WOW64; rv:24.0) Gecko/20100101 Firefox/24.0'}, proxies={})
...
def fix_pmid(ustring,online_check=False):
    if online_check:
        return fix_identifier(ustring,'pmid',online_method=pmid_has_content)
    else:
...
def fix_isbn(ustring,online_check=False):
    if online_check:
        return fix_identifier(ustring,'isbn',online_method=isbn_has_content)
    else:
...
def fix_issn(ustring,online_check=False):
    if online_check:
        return fix_identifier(ustring,'issn',online_method=issn_has_content)
    else:
...
def fix_scopusid(ustring,online_check=False):
    if online_check:
        return fix_identifier(ustring,'scopusid',online_method=scopusid_has_content)
    else:
...
def fix_ut(ustring,online_check=False):
    if online_check:
        return fix_identifier(ustring,'ut',online_method=ut_has_content)
    else:
